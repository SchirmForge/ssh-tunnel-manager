...
impl KnownHostEntry {
    ...
    fn parse(line: &str, line_number: usize) -> Option<Self> {
        ...
        if parts.len() < 3 {
            warn!(
                "Invalid known_hosts entry at line {}: too few fields",
                line_number
            );
            return None;
        }
    ...
    fn format(&self) -> String {
        if let Some(comment) = &self.comment {
            format!(
                "{} {} {} {}",
                self.host_pattern, self.key_type, self.key_data, comment
            )
        } else {
    ...
    pub fn load_from(path: &Path, hash_hostnames: bool) -> Result<Self> {
        ...
        if path.exists() {
            let file = fs::File::open(path).context(format!(
                "Failed to open known_hosts file: {}",
                path.display()
            ))?;
...
mod tests {
    ...
    fn test_format_host_pattern() {
        ...
        assert_eq!(
            format_host_pattern("192.168.1.1", 2222),
            "[192.168.1.1]:2222"
        );
    }
